...
export function useGroupQuiz({ groupId, sessionId }: UseGroupQuizProps) {
  ...
  const loadQuestionsFromShareTokens = useCallback(
    async (shareTokens: Record<string, string>) => {
      ...
      // ✅ FIXED: Sort by difficulty to ensure easy -> medium -> hard order
      const difficultyOrder = { easy: 0, medium: 1, hard: 2 };
      const formattedGroups: DifficultyGroup[] = loadedQuestions
        .sort((a: any, b: any) => {
      ...
      // console.log(
      //   '📚 Setting difficultyGroups in sorted order:',
  ...
  const submitGroupResultsMutation = useMutation({
    mutationFn: async (resultsData: any) => {
      ...
      const response = await fetch(
        `/api/groups/${groupId}/sessions/${sessionId}/submit`,
        {
          method: "POST",
          headers,
  ...
  const { shareTokens: existingShareTokens, isLoading: questionsLoading } =
    useSharedQuestions({
      groupId,
      sessionId,
  ...
  useEffect(() => {
    const loadExistingQuestionsFromCache = async () => {
      ...
      const statesNeedingQuestions = [
        "preset-selection",
        "question-preview",
        "quiz-active",
      ];
      const shouldLoadQuestions =
        statesNeedingQuestions.includes(appState) &&
      ...
      if (
        Object.keys(shareTokensToUse).length === 0 &&
        typeof window !== "undefined"
      ) {
        const storedShareTokens = sessionStorage.getItem(
          `quiz-shareTokens-${sessionId}`
        );
        if (storedShareTokens) {
          try {
            const parsedTokens = JSON.parse(storedShareTokens);
            // console.log(
            //   "📦 Found shareTokens in sessionStorage for members:",
            //   parsedTokens
            // );
            ...
            console.warn(
              "Failed to parse shareTokens from sessionStorage:",
              error
            );
          }
        }
      ...
      if (Object.keys(shareTokensToUse).length > 0) {
        ...
        try {
          ...
          if (appState === "preset-selection") {
            ...
            // console.log(
            //   `✅ Questions loaded for ${appState} state - preserving current appState`
            // );
          }
        } catch (error) {
        ...
        // console.log(
        //   "📭 No existing questions found in cache or session storage"
        // );
        ...
        if (
          ...
          //   appState,
          //   needsQuestions:
          //     appState === "quiz-active" || appState === "question-preview",
    ...
    (preset: QuestionPreset, questions: Question[]): DifficultyGroup[] => {
      ...
      difficulties.forEach((difficulty) => {
        ...
        if (questionCount > 0) {
          ...
          const questionsWithShuffledAnswers = selectedQuestions.map(
            (question) => {
    ...
    async (preset: QuestionPreset, shareTokens?: Record<string, string>) => {
      ...
      try {
        ...
        if (shareTokens) {
          ...
          const loadedQuestions = await loadQuestionsFromShareTokens(
            shareTokens
          );
          ...
          const firstQuestionSetWithVideo = loadedQuestions.find(
            (q: any) => q.questionSet.videoUrl
          );
  ...
  const handleStartQuizFromPreview = useCallback(
    async (currentShareTokens?: Record<string, string>) => {
      console.log(
        "🚀 [handleStartQuizFromPreview] Setting appState to quiz-active"
      );
      setAppState("quiz-active");
  ...
  const calculateFinalResults = async () => {
    ...
    for (let i = 0; i < difficultyGroups.length; i++) {
      ...
      const currentSetResponses = responses.filter(
        (r) =>
          r.questionIndex >= resultStartIndex && r.questionIndex <= setEndIndex
      );
    ...
    if (isAuthenticated && user?.id) {
      try {
        ...
        const response = await fetch(
          `/api/groups/${groupId}/sessions/${sessionId}/results`,
          {
            method: "POST",
            headers,
            body: JSON.stringify({
        ...
        if (!response.ok) {
          ...
          console.error(
            "Failed to submit final results to group leaderboard:",
            errorData
          );
        } else {
          console.log(
            "Final results submitted successfully to group leaderboard"
          );
        }
      } catch (error) {
  ...
  const navigateToQuestion = useCallback(
    (questionIndex: number) => {
      ...
      if (
        currentSet &&
        questionIndex >= 0 &&
        questionIndex < currentSet.questions.length
      ) {
