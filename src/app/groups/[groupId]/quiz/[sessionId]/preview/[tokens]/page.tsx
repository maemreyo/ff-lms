...
export default function PreviewWithTokensPage({
  params,
}: PreviewWithTokensPageProps) {
  const { groupId, sessionId, tokens } = use(params);
  const [decodedShareTokens, setDecodedShareTokens] = useState<
  ...
  useEffect(() => {
    ...
    if (tokens) {
      try {
        ...
        console.error(
          "❌ [Preview/Tokens] Failed to decode shareTokens from route:",
          error
        );
      }
    }
  ...
  const startQuizDirectly = useCallback(async () => {
    ...
    if (shareTokens && Object.keys(shareTokens).length > 0) {
      ...
      try {
        ...
        console.error(
          "❌ [Preview/Tokens] Failed to encode shareTokens:",
          error
        );
      ...
      console.warn(
        "⚠️ [Preview/Tokens] No shareTokens available, using regular active page"
      );
      activeUrl = `/groups/${groupId}/quiz/${sessionId}/active`;
