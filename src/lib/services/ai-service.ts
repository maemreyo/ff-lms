...
export class AIService {
  ...
  async generateConversationQuestions(
    ...
    if (options?.customPrompt) {
      ...
      if (options?.segments && options.segments.length > 0) {
        transcriptWithTimestamps = options.segments
          .map(
            (segment, index) =>
  ...
  async generateSingleDifficultyQuestions(
    ...
    while (
      ...
      try {
        ...
        const correctDifficultyQuestions = questions.filter(
          (q: any) =>
            // q.difficulty === difficulty &&
            q.question
          // && q.question.trim().length > 10
          //  && // Must have substantial question text
          // Array.isArray(q.options) &&
          // q.options.length === 4 &&
        ...
        for (const newQuestion of correctDifficultyQuestions) {
          ...
          const isDuplicate = finalQuestions.some((existing) => {
            ...
            return (
              existingText === questionText ||
              (existingText.length > 20 &&
      ...
      questions: finalQuestions.map((q: any, index: number) => {
        ...
        return {
          ...
          timestamp:
            q.timestamp ??
            loop.startTime +
              (index * (loop.endTime - loop.startTime)) / finalQuestions.length,
        } as GeneratedQuestion;
      }),
      preset: {
   ...
   */
  private shuffleOptionsWithSeed(
    options: string[],
    seed: string
  ): { options: string[] } {
  ...
  async generateQuestionsWithCustomPrompt(
    ...
    if (customPromptId) {
      const fetchedPrompt = await AIService.fetchCustomPrompt(
        customPromptId,
        supabaseClient
      );
        ...
        easyQuestions = await this.generateSingleDifficultyQuestions(
          ...
          "easy",
          {
            ...
            customPrompt: customPromptId
              ? (await AIService.fetchCustomPrompt(
                  customPromptId,
                  supabaseClient
                )) || undefined
              : undefined,
